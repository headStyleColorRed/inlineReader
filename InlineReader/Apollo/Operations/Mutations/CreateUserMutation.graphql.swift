...
extension API {
  class CreateUserMutation: GraphQLMutation {
    ...
    static let operationDocument: ApolloAPI.OperationDocument = .init(
      definition: .init(
        #"mutation CreateUser($email: String!, $password: String!) { createUser(email: $email, password: $password) { __typename user { __typename id email role name age } errors } }"#
      ))
    ...
    struct Data: API.SelectionSet {
      ...
      /// CreateUser
      ///
      /// Parent Type: `CreateUserPayload`
      struct CreateUser: API.SelectionSet {
        let __data: DataDict
        init(_dataDict: DataDict) { __data = _dataDict }

        static var __parentType: any ApolloAPI.ParentType { API.Objects.CreateUserPayload }
        static var __selections: [ApolloAPI.Selection] { [
